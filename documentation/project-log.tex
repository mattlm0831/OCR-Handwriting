...
\item Although, on first glance, it appears that the validation accuracy was pretty good (although heavily over-fitted), yet the loss remained above 75\% for the duration of the training. I think that this means a few things, firstly we need to maket the network deeper and use more advanced \textbf{layer techniques}. Secondly, adding augmentation will certainly take a lot of that loss, and over-fitting away. These augmentations to the model will happen next Tuesday.
\end{enumerate}
